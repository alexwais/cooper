...
//        state.getCurrentSystemMeasures().getTotalServiceLoad().forEach((serviceName, currentLoad)  -> {
...
//                        Map.Entry::getKey,
//                        e -> e.getValue().stream().map(ContainerInstance::getType).collect(Collectors.toList())
//                ));
//
...
//
//        return new OptimizationResult(model, state.getCurrentSystemMeasures(), alloc);
//    }
//
//
...
//    }
//
//    private List<Allocation.AllocationTuple> deallocateOverprovisionedResources(String serviceName, long delta) {
//        var service = model.getServices().get(serviceName);
//        var runningContainersIterator = state.getCurrentTargetAllocation().getAllocatedContainersByService().get(service).stream().iterator();
//
//        var result = new ArrayList<Allocation.AllocationTuple>();
//        while (delta < 0) {
//            var a = runningContainersIterator.next();
//            var isNotReleasable = delta + a.getContainer().getRpmCapacity() > 0;
//            if (isNotReleasable) {
//                break;
//            }
//            result.add(a);
//            delta += a.getContainer().getRpmCapacity();
//        }
//
//        return result;
//    }
