...
//
//@Slf4j
//public class AlternativeFirstFitOptimizer implements Optimizer {
//
//    private final Model model;
//    private final Validator validator;
//
//    public AlternativeFirstFitOptimizer(Model model) {
//        this.model = model;
...
//
//                var requiredContainerType = getNextFittingContainerType(service, missingCapacity);
//
//                additionalNeededContainerInstances.add(requiredContainerType);
//                remainingCapacity -= requiredContainerType.getRpmCapacity();
//            }
//        }
//
//
...
////                }
//
//                var isServiceAlreadyPresent = containers.stream().map(ContainerType::getService).collect(Collectors.toList())
//                        .contains(container.getService());
//                if (isServiceAlreadyPresent) {
//                    remainingAdditionalContainers.add(container);
//                    continue;
//                }
//
...
//            log.warn("No available VM remaining!");
//        }
//
...
//
//
//        var requiredContainerType = getNextFittingContainerType(service, missingServiceCapacity);
//
//        // all currently allocated + previously allocated containers (which are now in grace period)  must fit on the VM
//        var containersToTest = new ArrayList<>(allocatedContainers);
//        containersToTest.add(requiredContainerType);
//        var containerFits = !validator.isVmOverallocated(vm, containersToTest, previousContainers);
//        if (containerFits) {
//            return requiredContainerType;
//        } else {
//            return null;
//        }
//
...
//
//
//    private ContainerType getNextFittingContainerType(Service service, Long missingServiceCapacity) {
//        // determine next larger container type to required capacity
//        var nextBiggest = service.getContainerTypes().stream()
//                .filter(c -> c.getRpmCapacity() >= missingServiceCapacity)
//                .min(Comparator.comparingLong(ContainerType::getRpmCapacity));
//
//        // if no larger type is available, we use the largest possible for this service
//        var biggestAvailable = service.getContainerTypes().stream()
//                .max(Comparator.comparingLong(ContainerType::getRpmCapacity));
//
//        return nextBiggest.orElseGet(biggestAvailable::get);
//    }
//
//}
